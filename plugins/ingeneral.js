...
ezra({
  ...
  reaction: '🚀',
  categorie: 'General-Chris'
}, async (dest, zk, commandeOptions) => {
  const { repondre, arg, ms } = commandeOptions;

  if (!arg || arg.length === 0) {
    const replyText = "Example Usage: .code 2348069xxxxxx.";
    return repondre(replyText);
  }

  try {
    // Notify user that pairing is in progress
    const replyText = "*Wait CHRISSPARK-XMD is getting your pair code ...*";
