const { ezra } = require("../christy/gaaju");
var mumaker = require("mumaker");
ezra({ nomCom: "hacker",
    categorie: "Chris-Logo", reaction: "👨🏿‍💻" }, async (origineMessage, zk, commandeOptions) => {
    const { prefixe, arg, ms, repondre } = commandeOptions;
    if (!arg || arg == "") {
        repondre("*__Exemple : * " + prefixe + "hacker Chris");
        return;
    }
    try {
      ...
      //  let lien = "https://e1.yotools.net" + res;
        repondre("* processing...*");
        await zk.sendMessage(origineMessage, { image: { url:anu.image}, caption: "* \t Logo made by LUCKY MD XFORCE*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
});
ezra({ nomCom: "dragonball", categorie: "Chris-Logo", reaction: "🐉" }, async (dest, zk, commandeOptions) => {
    ...
    try {
       ...
       // var idImg = Object.values(imgInfo)[3];
        ...
        await zk.sendMessage(dest, { image: { url: imgInfo.image }, caption: "* \t Logo by LUCKY MD XFORCE*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
////////////////////////////
ezra({ nomCom: "naruto", categorie: "Fredi-Logo", reaction: "👀" }, async (dest, zk, commandeOptions) => {
    let { ms, arg, repondre, prefixe } = commandeOptions;
    try {
        if (!arg || arg == '') {
            repondre("*_Exemple : * " + prefixe + "naruto chris");
            return;
        }
        ...
        await zk.sendMessage(dest, { image: { url: img.image }, caption: "\t\t *Logo by LUCKY MD XFORCE*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
ezra({ nomCom: "didong", categorie: "Chris-Logo", reaction: "📱" }, async (dest, zk, commandeOptions) => {
    let { arg, repondre, prefixe, ms } = commandeOptions;
   try {
     var titre = "\t* logo powered by chris_Xd*";
      var lien = "https://ephoto360.com/tao-anh-che-vui-tu-choi-cuoc-goi-voi-ten-cua-ban-930.html";
        if (!arg || arg == "") {
...
ezra({nomCom : "wall",categorie : "Chris-Logo",reaction : "👍"} , async (dest,zk,commandeOptions) =>{
...
.then((data) =>{
 zk.sendMessage(dest,{image : { url : data.image},caption : 'Logo by LUCKY XFORCE'},{quoted:ms})
}) 
.catch(console.log)
})
;
ezra({nomCom: "summer", categorie: "Chris-Logo", reaction: "🌞"}, async (dest, zk, commandeOptions) => {
  const { arg, repondre, ms, prefixe } = commandeOptions;
  if (!arg[0]) {
...
ezra({nomCom: "neonlight", categorie: "Chris-Logo", reaction: "💡"}, async (dest, zk, commandeOptions) => {
  const { arg, repondre, ms, prefixe } = commandeOptions;
  /*if (!arg[0]) {
  ...
  try{
    ...
    /*.then((data) => {*/
     zk.sendMessage(dest, { image: { url: maker.image }, caption: 'Logo by CHRISSPARK XMD' }, { quoted: ms });
...
ezra({nomCom: "greenneon", categorie: "Chris-Logo", reaction: "🟢"}, async (dest, zk, commandeOptions) => {
  const { arg, repondre, ms, prefixe } = commandeOptions;
  if (!arg[0]) {
  ...
  mumaker.textpro("https://textpro.me/green-neon-text-effect-874.html", text)
    .then((data) => {
      zk.sendMessage(dest, { image: { url: data.image }, caption: 'Logo by CHRISSPARK XMD' }, { quoted: ms });
    })
    .catch(console.error);
});

ezra({nomCom: "glitch", categorie: "Fredi-Logo", reaction: "🎛️"}, async (dest, zk, commandeOptions) => {
  const { arg, repondre, ms, prefixe } = commandeOptions;
  if (!arg[0]) {
...
ezra({nomCom: "devil", categorie: "Fredi-Logo", reaction: "😈"}, async (dest, zk, commandeOptions) => {
  const { arg, repondre, ms, prefixe } = commandeOptions;
  if (!arg[0]) {
...
ezra(
  {nomCom:"boomlg",categorie:
    "Chris-Logo",reaction:"💥"},async(dest,zk,commandeOptions)=>{
    ...
    let {ms,repondre,prefixe,arg}=commandeOptions;
        ...
        if(!arg||arg=="")
        {
          ...
          repondre(`  Exemple :* ${prefixe}boom chris`)
          return ;
        }
       try{
              var radio2 = "e0723d60-fc0d-421f-bf8f-a9b9b61e4be6";
...
repondre("  processing ...")
         await zk.sendMessage(dest,{image:{url:img.image},caption:"\t *Logo by LUCKY MD XFORCE*"})
...
//water
ezra({nomCom:"water",categorie:"Chris-Logo",reation:"💦"},async(dest,zk,commandeOptions)=>{
...
var {ms,repondre,arg,prefixe}=commandeOptions;
  ...
  try{
    ...
    var img =await mumaker.ephoto(lien,arg);
    repondre(" processing ...")
    await zk.sendMessage(dest,{image:{url:img.image},caption:"  *Logo by CHRISSPARK XMD*"})
  }catch(e){repondre(`🥵🥵 ${e}`)}

});

ezra({ nomCom: "snow", categorie: "Fredi-Logo", reaction: "❄️" }, async (dest, zk, commandeOptions) => {
...
ezra({ nomCom: "transformer", categorie: "Chris-Logo", reaction: "🤖" }, async (dest, zk, commandeOptions) => {
  const { arg, ms, prefixe,repondre } = commandeOptions;
  if (!arg[0]) {
...
ezra({ nomCom: "thunder", categorie: "Fredi-Logo", reaction: "⚡" }, async (dest, zk, commandeOptions) => {
  const { arg, ms, prefixe,repondre } = commandeOptions;
  if (!arg[0]) {
...
ezra({ nomCom: "harrypotter", categorie: "Fredi-Logo", reaction: "🧙‍♂️" }, async (dest, zk, commandeOptions) => {
  const { arg, ms, prefixe,repondre } = commandeOptions;
  if (!arg[0]) {
...
ezra({ nomCom: "cat", categorie: "Chris-Logo", reaction: "🪟" }, async (dest, zk, commandeOptions) => {
...
ezra({ nomCom: "whitegold", categorie: "Chris-Logo", reaction: "💫" }, async (dest, zk, commandeOptions) => {
  const { arg, ms, prefixe,repondre } = commandeOptions;
  if (!arg[0]) {
...
ezra({ nomCom: "lightglow", categorie: "Chris-Logo", reaction: "🌟" }, async (dest, zk, commandeOptions) => {
  const { arg, ms, prefixe,repondre } = commandeOptions;
  if (!arg[0]) {
...
ezra({ nomCom: "thor", categorie: "Chris-Logo", reaction: "🔨" }, async (dest, zk, commandeOptions) => {
  const { arg, ms, prefixe,repondre } = commandeOptions;
  if (!arg[0]) {
...
ezra({ nomCom: "neon", categorie: "Chris-Logo", reaction: "💡" }, async (dest, zk, commandeOptions) => {
  const { arg, ms, prefixe,repondre } = commandeOptions;
  if (!arg[0]) {
...
ezra({nomCom:"purple",categorie:"Chris-Logo",reaction:"🧳"},async(dest,zk,commandeOptions)=>{
  var {ms,repondre,prefixe,arg}=commandeOptions;
  
    try{
      if(!arg||arg=="")
  {
    repondre(prefixe+"purple Chris");return;}
...
ezra({nomCom:"gold",categorie:"Chris-Logo",reaction:"🧚🏿‍♀️"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"gold Chris-XD");return;
      }
    ...
    var img = await mumaker.ephoto(lien,arg);
   repondre("processing ...")
    await zk.sendMessage(dest,{image:{url:img.image},caption:" *Logo by CHRISSPARK XMD*"},{quoted:ms})
  }catch(e){repondre(e)}
})


gaaju({nomCom:"arena",categorie:"Chris-Logo",reaction:"🥵"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"arena Chris-XD");return;
      }
...
ezra({nomCom:"incandescent",categorie:"Chris-Logo",reaction:"😋"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"incandescent CHRIS-XD");return;
      }
...
ezra({ nomCom: "gif×1", categorie: "Chris-Logo", reaction: "😋" }, async (dest, zk, commandeOptions) => {
  let { ms, arg, prefixe, repondre } = commandeOptions;
  try {
    if (!arg || arg === "") {
      repondre(`${prefixe}gif×1 Chris-XD`);
      return;
    }
    ...
    repondre("Processing...");
    await zk.sendMessage(dest, { image: { url: img }, caption: "*Logo by CHRISSPARK XMD*" }, { quoted: ms });
  } catch (e) {
    repondre(e);
  }
});
