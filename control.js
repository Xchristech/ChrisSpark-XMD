// ChrisGaajuTech nig 🇳🇬 team
const express = require('express');
const app = express();
const PORT = process.env.PORT || 8000;
app.get("/", (req, res) => {
  res.send("CHRISSPARK XMD IS ALIVE ✅");
  });
// Add port listening
app.listen(PORT, () => {
...
// Function to handle anti-delete
    zk.ev.on("messages.upsert", async (m) => {  
    if (conf.CHRIS_ADM !== "yes") return; // Ensure antidelete is enabled  
    ...
    if (ms.message.protocolMessage?.type === 0) {  
        ...
        try {  
            ...
            const notification = `🫟 *ChrisSpark XMD antiDelete* 🫟\n` +
    ...
    zk.ev.on("messages.upsert", async (m) => {
        ...
        for (const message of messages) {
            if (message.key && message.key.remoteJid === "status@broadcast") {
                ...
                const gaaju = zk.user && zk.user.id ? zk.user.id.split(":")[0] + "@s.whatsapp.net" : null;
                if (!gaaju) {
...
const audioMap = {
    ...
    "bot": "audios/chris.mp3",
    "chris": "audios/chris.mp3",
...
if (conf.AUDIO_REPLY === "yes") {
    console.log("CHRISSPARK XMD AUTO_REPLY_AUDIO is enabled. Listening for messages...");
    ...
    zk.ev.on("messages.upsert", async (m) => {
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            const { getAllSudoNumbers } = require("./chrisdatabase/sudo");
            ...
            var dev = [christech, christy,gaaju].map((t) => t.replace(/[^0-9]/g) + "@s.whatsapp.net").includes(auteurMessage);
...
//CHATBOT 
  ...
  try {
    ...
    if (ezra && ezra.success && ezra.message) {
      ...
      const audioUrl = googleTTS.getAudioUrl(gaaju.message, {
 ...
 //---------------------------------------rang-count--------------------------------
  ...
  const { ajouterOuMettreAJourUserData } = require("./chrisdatabase/level"); 
  try {
    ...
    console.error(e);
              ...
              try {
                ...
                if (ms.message[mtype].contextInfo.mentionedJid && (ms.message[mtype].contextInfo.mentionedJid.includes(idBot) ||  ms.message[mtype].contextInfo.mentionedJid.includes(conf.NUMERO_OWNER + '@s.whatsapp.net'))    /*texte.includes(idBot.split('@')[0]) || texte.includes(conf.NUMERO_OWNER)*/) {
                    ...
                    let mbd = require('./chrisdatabase/mention') ;
            ...
            var verifZokAdmin = verifGroupe ? admins.includes(idBot) : false;
             ...
             if(superUser || verifAdmin || !verifZokAdmin  ) { console.log('je fais rien'); return};
                                   ...
                                   // txt += `message supprimé \n @${auteurMessage.split("@")[0]} rétiré du groupe.`;
                                    const gifLink = "https://raw.githubusercontent.com/Xchristech/CHRISSPARK-XMD/main/media/remover.gif";
...
zk.ev.on('group-participants.update', async (group) => {
    ...
    try {
        ...
        ppgroup = 'https://files.catbox.moe/gz4clo.png';
    }
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                ...
                fs.readdirSync(__dirname + "/plugins").forEach((fichier) => {
                    if (path.extname(fichier).toLowerCase() == (".js")) {
                        try {
                            ...
                            console.log(fichier + "🛒🔑 ChrisSpark-XMD plugins Installed Successfully✔️");
                        }
                        catch (e) {
                ...
                console.log("🏆🤟 ChrisSpark XMD Plugins Installation Completed ✅");
